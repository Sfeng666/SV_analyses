...
while read -r name_sample name_sample_new; do
    ...
    then
        tar -zxvf $path_tar/05realign_$name_sample\.tar.gz -C $path_out/$name_sample_new $(basename $in_bam_original)
...
if ! [ -f $bam_contig_merged_genozip ];
then
  genozip --no-test -^ --vblock 200 $bam_contig_merged
