...
then
    ...
    | samtools sort -l 0 -n - \
    | bedtools pairtobed -ubam -abam stdin -b $bed_split_distant \
    | samtools view -o $sam_contig_merged_distant - >> $log 2>&1 && echo -e "### preparing split merged sam input for del ends at" `date +%F'  '%H:%M` "\n" >> $log &

    ## for everted inserts
    awk 'NR == 2{print}' $bed_split > $bed_split_everted
    samtools merge -u - $bam_contig_allsample \
    | samtools sort -l 0 -n - \
    | bedtools pairtobed -ubam -abam stdin -b $bed_split_everted \
    | samtools view -o $sam_contig_merged_everted - >> $log 2>&1 && echo -e "### preparing split merged sam input for dup ends at" `date +%F'  '%H:%M` "\n" >> $log
else
    samtools merge -u - $bam_contig_allsample \
...
rm -f $bam_contig_allsample *tmp*.bam
...
if $large
then
    cat $sam_contig_merged_distant | python $dir_poolDiffCNV/findEvertedInserts.py > $out_everted_inserts # find discordant read pairs for duplications
else
    cat $sam_contig_merged | python $dir_poolDiffCNV/findEvertedInserts.py > $out_everted_inserts # find discordant read pairs for duplications
fi
...
if $large
then
    cat $sam_contig_merged_everted | python $dir_poolDiffCNV/findDistantInserts.py $insert_size_cutoff > $out_distant_inserts # find discordant read pairs for deletions
else
    cat $sam_contig_merged | python $dir_poolDiffCNV/findDistantInserts.py $insert_size_cutoff > $out_distant_inserts # find discordant read pairs for deletions
fi
